...
Restore an **authorized typed exact-source test/patch route to the existing `/tmp/agentfield-f06-fix` owner clone** on the current workforce, without creating a duplicate workspace/runtime and without GitHub-edit -> redeploy. The current registry does not expose that path through stale-safe file operations and operator mediation blocks generic `go test`/mutation; any persistent widening of target write scope must be explicitly authorized before registry mutation. Once the existing clone is safely reachable, resume this same takeover gate: reproduce RED on every applicable P0 matrix boundary, reconcile/backport only evidence-backed behavior (using #1000/#1001/#1004/#1011/#1046 as references rather than a wholesale upgrade), run targeted `-race` + full Go/control-plane/Python-as-changed suites + `git diff --check`, re-index with Codebase Index + Graphify, run one BMAD adversarial/edge review, create one exact local tested AgentField commit, validate it against frozen SWE and repeat the restart fault. Only after fresh GREEN evidence and SoT write-back may the critical path return to **FB-0 full `swe-planner.build` attempt 5**.
