...
The sole next move is to satisfy the enforced **`DEBUG_CLONE_APPROVAL_REQUIRED`** gate for the already-prepared bounded request against exact `agentfield-dev-workforce` source/image, then immediately execute the clone and run F16 RED. Do not bypass approval with generic Docker/terminal mutation. Because approval requests are short-lived and the normal workforce bootstrap is `Restarting (31)`, re-observe source-container identity before preparing any replacement request; approval must remain bound to the fresh source identity. After approved clone creation: prove `/usr/local/go/bin/go` and exact `/src/swe-af`, add only the corrupt-checkpoint discriminator, obtain RED, then apply the smallest fail-closed fix and run targeted DAG/orch tests + full `go test ./... -count=1` + `git diff --check`, one adversarial/edge review, verified clone cleanup, and PLAN write-back. Product programming remains container-first; GitHub is publication/SoT, not the coding loop.
