...
CURRENT BLOCKER:
AgentField #1011, #1000, #1001, heartbeat/version compatibility, and #1004 persisted generation ownership are now **GREEN on the exact owner clone**. #1004 now uses the native persistence/lifecycle design: target revision `4` added only `canary_file`; canonical migration `035_execution_instance_id.sql` exists; execution + workflow models/types persist serving `instance_id`; execution/workflow SQL round-trips it; new dispatch stamps `agent.InstanceID`; orphan reap is instance-scoped and includes explicit legacy rows while preserving replacement-generation work; re-registration defers cleanup through native drain grace instead of synchronously reaping the whole node. Fresh `internal/storage`, `internal/handlers`, and `internal/server` suites PASS. The active P0 is #1046: stale workflow cleanup must honor recent activity on its paired execution and must re-check staleness at the final UPDATE boundary.

THIS BATCH:
reconciled SoT/runtime, researched current external skill guidance and retained only root-cause/TDD/verification practices that add signal, then executed container-first. Added `canary_file` narrowly to the existing target (revision `4`, no new roots/runtime/workspace), created migration `035`, drove schema RED->GREEN, ported only generation-related #1004 native behavior from upstream, added persistence/selective-reap oracles, fixed affected SQL-shape/Postgres fixtures, and reran full storage/handlers/server suites GREEN. The optional configurable `AgentDrainGrace` config-schema extension is still policy-blocked by `CANONICAL_OWNER_BEFORE_CONFIG`; the product path uses the upstream native 60s default via atomic drain-grace state, so this extension is not required for the #1004 behavior gate. Target-owned `git_diff_check` is also mis-rooted at `/src` and fails before inspecting the exact clone; classify that as validator-path blocker, not product FAIL.

NORTH-STAR DELTA:
restart recovery now distinguishes process generations in persisted state and cannot node-wide reap a healthy replacement generation. The takeover matrix has one remaining active data-convergence P0 (#1046) before final graph/adversarial review, exact local commit, frozen-SWE validation and real restart re-injection.

STOP CONDITION:
close #1046 with deterministic RED->GREEN for paired execution activity and selection-to-update races; then rerun relevant full suites/race evidence, re-index exact source when callable, run final BMAD adversarial/edge review, produce one exact tested local AgentField commit, validate frozen SWE and repeat the restart fault injection. Do not start FB-0 before that evidence gate is GREEN.
