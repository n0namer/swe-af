...
Promote the independently validated temporary upstream-cutover branch to the single `dev` integration line without PR/CI/deploy, guarded by fresh remote-head readback. The accepted `dev` must contain upstream `311f376a2f12df01134acd80384d599dd8039178`, preserved `resume_build_id`, and the replayed coder nested-module CWD/prompt patch; superseded OpenCode overlay/venv behavior must remain absent. After `dev` publication, do not rewrite SourceLoop binding/provenance yet: the running `/src/swe-af` generation is still old-base `6f5b438...`, so fresh captures remain old-generation queue items until a separate runtime cutover is authorized and read back. No deploy/redeploy, PR, or CI is part of this source cutover.
