...
CURRENT BLOCKER:
The active P0 is now **F16 fail-closed resume on corrupted checkpoint** in SWE exact source, with a secondary `VALIDATION_BLOCKER` because the currently reachable source-observation container has no Go toolchain. Static owner-source readback proves the defect: `loadCheckpoint()` returns `nil` for both missing/unreadable and malformed JSON; when `RunDAG(..., WithResume(true))` receives `nil`, it silently keeps a freshly initialized DAG state and immediately writes a new checkpoint. A corrupt/partial checkpoint is therefore treated as “no checkpoint”, which can replay already-completed non-idempotent effects after crash/restart. Existing tests cover missing checkpoint and valid round-trip/resume, but there is no discriminating corrupt-checkpoint fail-closed oracle.

THIS BATCH:
reconciled the dirty `/src/swe-af` source against the P0 risk matrix instead of touching services. The first still-open pure source-level P0 is F16; F03/F04 real OpenCode and F05/F06 external restart/provider faults remain integration-layer work and do not justify more unit patches before code convergence. The exact RED contract is now defined: write malformed `artifacts/execution/checkpoint.json`, call `RunDAG(..., WithResume(true))`, and require an error before any `ExecuteFn` call or checkpoint overwrite. No production mutation was made because executable RED cannot currently run: `agentfield-dev-runtime-capture` exposes the exact source but has no Go binary; Coding Station is `Gateway Timeout`; the running Coding Station runtime also has no Go toolchain; another workbench runner was not used because scope binding is unproven.

NORTH-STAR DELTA:
this converts the vague “code still incomplete” state into one concrete high-impact source defect with a precise executable oracle. Fixing F16 closes the remaining checkpoint-corruption replay hole on the autonomous recovery path and materially advances behavior completeness without service/runtime side work.

STOP CONDITION:
do not patch F16 until the discriminating RED runs on the exact source (or an exact validated source snapshot) with a Go runner. Once RED is available, make the smallest owner-layer change so resume distinguishes missing checkpoint from corrupt/unreadable checkpoint and fails closed before mutation/overwrite; then run targeted DAG/orch tests, full canonical Go suite, `git diff --check`, and one adversarial edge review. Do not resume service/runtime/canonicalization work until F16 is GREEN and the remaining source-level P0 inventory is rechecked.
...
Do **not** spend the next batch on service startup, native package rollout, canonical writeback, or another FB-0 run. The next 30-minute batch is a **code-convergence batch** on exact SWE source. Reconcile the current `/src/swe-af` working-tree delta against the proven fault/risk evidence and classify only the still-open North-Star P0 behaviors. For each real gap, require a discriminating executable RED before implementation where practical, apply the smallest owner-layer fix, then run targeted package tests plus the full canonical Go suite and `git diff --check`. Reuse BMAD `test-design`/`trace` to maintain the risk matrix and `quick-dev` only for a proven source defect; finish with one adversarial/edge review of the final exact code delta. Stop the batch when either (a) one P0 code gap is closed with fresh GREEN evidence, or (b) all currently proven P0 code gaps are executable-green and a single exact code-complete candidate is identified. Only case (b) unlocks SourceLoop/Git publication, durable runtime materialization and a new full-Build acceptance attempt.
