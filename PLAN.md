...
For a new AI/operator, run the **AI Handoff / Bootstrap Protocol** first. Reconcile `/tmp/agentfield-f06-fix` from HEAD `b160245833ec51f5296905c32e49260a62c76e26` plus its uncommitted graph-assisted delta; complete discriminating tests/race/full suites, re-run Codebase Index + Graphify boundary discovery, perform one BMAD adversarial/edge review, create one exact local tested AgentField commit, validate it against frozen SWE and repeat the restart fault if process containment changed. Update this SoT from fresh evidence. Only then, if no higher-priority blocker remains, restore the critical path to **FB-0 full `swe-planner.build` attempt 5**.
