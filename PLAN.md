...
7. Run one non-mutating reasoner/schema smoke with execution evidence and confirm live `implement_issue` callability. **BLOCKED BY EXECUTION CONTROL, NOT SWE LIVENESS**: external AgentField Actions currently return `no available server`; DEV vps-terminal mediation classifies even `af list nodes` as opaque. Server-side live-patch helper for the operator fails closed with `live_patch_invalid_output`, and `/app/gateway` is mounted read-only, so this chat cannot safely add an ad-hoc allow-rule in place. Production vps-terminal does not currently register `agentfield-dev-workforce`, and its `coolify-control` target lacks a Docker CLI. Do not use GitHub-first/redeploy or shell-classifier bypasses; continue once a native execution channel is callable.
