...
CURRENT implementation state (2026-09-07 anti-drift): the earlier WRITEBLOCKED statement was stale. The CURRENT target registry already grants `agentfield-dev-workforce` `live_patch` with root `/src`; only creation of a new file was denied. The Pareto implementation therefore stayed inside existing owners and was applied stale-safe directly to the persistent DEV source `/src/swe-af` with no GitHub coding/redeploy. Batch 1 Shadow Decision Run now lives in `swe_af/hitl/wrapper.py`, is wired into all four existing `run_with_ask_user()` production call sites (`product_manager`, `environment_scout`, `issue_advisor`, `replanner`), and is covered by bounded tests in `tests/test_ask_user.py`. It invokes the existing harness with only `Read`/`Glob`/`Grep`, emits a structured `HITL_DECISION_EVENT`, never auto-resumes in this batch, and falls through to the existing HAX human path on resolver failure. Exact shadow tests PASS `2/2`; scoped `git diff --check` PASS. Canonical `make check` remains a VALIDATION_BLOCKER before SWE tests because this container has a stale editable `evalguard` pytest entrypoint pointing at a removed `/tmp/...` source, and the runtime also lacks declared test/runtime packages `pytest-asyncio`/`hax`; those environment defects are not SWE application failures. After live validation, the accepted four-file delta was canonicalized to Git `dev` as publication-only write-back (no redeploy); current head is `1b1395869e074c32eb263d5fe4566167e4fd865b`. SourceLoop still reports its live-patch journal entries `PENDING` because its `CANDIDATE` transition requires a `pr_number` even for an already-published direct `dev` write-back; do not create a fake PR solely to satisfy journal metadata. Runtime activation is the remaining evidence gap: AgentField gateway health is `ok`, but reasoner/node discovery currently returns `Bad Gateway`, and mediation blocks direct internal control-plane HTTP reads. The workforce target itself is a source/provenance host (`python3 -m http.server ... /afhome`), so container health/restart would not prove that a new reasoner execution loaded this source.
