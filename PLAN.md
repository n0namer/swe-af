...
- Current gate before Attempt 8: keep frozen source `6f5b4382e6231721f60be7045b9d91fd85e34fb5`, AgentField `c0923acdfca043c2c07e3d34daaa09e2a7e41d38`, explicit frozen Build config and the 0/3 streak. Reuse or materialize the **native SWE runtime contract** rather than adding more planning/schema code. Before any new Build START, executable preflight must prove (a) the loaded planner runtime has a real OpenCode binary via native PATH or explicit `SWE_OPENCODE_BIN`, (b) the same runtime carries broker env without disclosure, and (c) a non-mutating direct OpenCode `fcm/fcm` canary through `OpenCodeNoInstallPermissionOverlay` produces at least one real model response/turn. If no existing authorized SWE runtime can provide this, record `RUNTIME_MATERIALIZATION_GAP`; do not keep rerunning Build in the workforce shell.
...
CURRENT BLOCKER:
AgentField takeover remains **VERIFIED/CLOSED** at `c0923acdfca043c2c07e3d34daaa09e2a7e41d38`. The active P0 is **FB-0 Attempt 7** on repaired frozen SWE `6f5b4382e6231721f60be7045b9d91fd85e34fb5` with exact planner `/tmp/swe-planner-fb0-6f5b438-c092` SHA256 `3dc66b5d7f727f1b3e973e8e2fe6b7b29004853d7079f26fa599e13bacab5cd3`. Attempts 5/6 produced pre-task evidence, not accepted task runs: Attempt 5 exposed omitted explicit model/runtime config; Attempt 6 then exposed a real PM/OpenCode framework parity gap where the runtime-owned FCM provider overlay was not passed to Product Manager. That gap is now RED->GREEN repaired, full-suite validated in CURRENT and frozen source, and committed in the frozen stack. Current planner instance `89e4bc05f9c34a74a6e73c5f072f1531` is ready on localhost; frozen repo is clean and there are zero live OpenCode processes (defunct zombies only).

THIS BATCH:
preregistered the autonomy experiment manifest/task ladder before running FB-0; made Build route/model explicit to eliminate ambient-env selection bias; executed Attempt 5 and localized config drift; executed Attempt 6 on the corrected route and localized the first real framework blocker before model progress. BMAD test-design + Quick Dev and systematic-debugging/TDD were then applied to one contract: OpenCode PM must receive the same `OpenCodeNoInstallPermissionOverlay` already owned by SWE for coding/advisor roles. Existing `TestProductManagerOpenCodeUsesHarnessWhenDirectAIIsAvailable` was strengthened and went RED exactly on missing `OPENCODE_CONFIG_CONTENT`; one bounded production change injects the overlay only for OpenCode PM; targeted planning and full Go suites PASS in `/src/swe-af`; the exact two-file delta was propagated to frozen source, targeted/full suites and diff checks PASS, then committed as `6f5b4382...`. Planner was rebuilt against exact AgentField c092 and loaded as a fresh generation.

NORTH-STAR DELTA:
the first autonomous-Build experiment is now preregistered against drift and has already converted two failed starts into reusable evidence: ambient config is no longer allowed to choose the model/runtime, and the PM OpenCode route now owns the same broker-provider contract as downstream coding roles. The next run begins on a newly frozen exact stack rather than an operator-modified in-flight task.

STOP CONDITION:
start Attempt 7 only with clean frozen repo, fresh ready planner identity, zero live mutator, and the exact frozen Build config. From START to terminal make no task-code/prompt/model/provider/retry/oracle changes. If Attempt 7 reaches terminal, either independently accept it as task 1/3 or make its first causally proven framework blocker the sole next move; any framework repair resets the streak to 0/3.
...
Start **FB-0 Attempt 7** on the newly frozen stack: SWE `6f5b4382e6231721f60be7045b9d91fd85e34fb5`, AgentField `c0923acdfca043c2c07e3d34daaa09e2a7e41d38`, planner `/tmp/swe-planner-fb0-6f5b438-c092` SHA256 `3dc66b5d7f727f1b3e973e8e2fe6b7b29004853d7079f26fa599e13bacab5cd3`, and explicit Build config `{"runtime":"open_code","models":{"default":"fcm/fcm"},"enable_github_pr":false,"check_ci":false}`. Fresh precondition already shows a clean frozen repo, zero live OpenCode mutators and ready planner instance `89e4bc05f9c34a74a6e73c5f072f1531`; re-read these immediately before START rather than trusting this text. From Build START to terminal state perform **zero operator task-code edits and zero prompt/model/provider/config/retry/oracle changes**. After terminal state, run canonical Go validation + the preregistered independent task oracle and record exact final source/artifact identity, wall time, model/cost telemetry when available, recovery/retry counts, UNKNOWN states and duplicate effects. If accepted, record full-Build streak 1/3; otherwise preserve the first causally proven framework blocker as the sole next move and reset the streak after repair.
