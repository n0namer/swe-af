...
Do **not** search for a new Go runner or create a substitute validation environment. The sole next move is **F16 corrupt-checkpoint fail-closed RED->GREEN using the already-proven SWE/workforce validation environment**. Re-observe `workforce-edshqtkwskg3lrczekhcmd71-021517011499` and make that existing environment available for code-only validation through its supported owner/lifecycle route without treating the action as production acceptance. Then add one discriminating DAG regression on exact `/src/swe-af`: create malformed `artifacts/execution/checkpoint.json`, call `RunDAG(..., WithResume(true))`, require a non-nil error before any `ExecuteFn` call, and prove the corrupt checkpoint bytes are not overwritten. Only after RED may implementation change checkpoint/resume handling so missing and corrupt/unreadable states are distinct and corrupt resume fails closed. Then run targeted DAG/orch tests, full `/usr/local/go/bin/go test ./... -count=1`, `git diff --check`, and one adversarial/edge review. Do not spend the batch on unrelated runtime rollout, canonical publication or FB-0 acceptance.
