...
CURRENT BLOCKER:
The DEV exact-source capability blocker is **closed**. Canonical `n0namer/vps-terminal` PR #160 was merged as `9cf1f189e02df1827440494bc536c2deb23d3ea7`; its tested candidate `cba00692b3d791902eebcce94cb6560dc2a03c58` proved additive target-registry allowlisting and trusted absolute Go test-runner classification. `agentfield-dev-workforce` is now registry revision `3`, preserves its original selector/capabilities, has `live_patch_roots=["/src","/tmp/agentfield-f06-fix"]`, and uses `live_patch_runtime="python"` because the workforce has Python but no Node. Stale-safe exact-source read/patch works and `/usr/local/go/bin/go test ...` executes through the typed VERIFY lane. The remaining blocker is product correctness: #1011 is GREEN, while #1000 graceful accepted-work drain now has a deterministic RED. FB-0 remains blocked until #1000/#1001/#1004/#1046 and the remaining P0 matrix close.

THIS BATCH:
used BMAD test-design/quick-dev/adversarial-edge/trace discipline to repair the supporting operator route and resume real AgentField TDD. Supporting operator evidence: exact candidate runtime identity `cba00692...`, changed-file `node_check` PASS, revision-guarded registry upsert/readback PASS, absolute Go runner functional oracle PASS; GitHub Actions remains baseline `startup_failure` with 0 jobs and `npm run check` is environment-blocked before tests by missing baseline fixture `/var/lib/vps-terminal/gateway-hotfix/memory-emitter.mjs`, so these are recorded as validation-plane blockers rather than candidate FAIL. AgentField #1011 got a new discriminating notify-window test, genuine RED, minimal `notify -> close admission -> cancel` GREEN, targeted shutdown tests PASS, targeted `-race` PASS, and full `sdk/go ./...` PASS. Next, #1000 got a new discriminating drain test and genuine RED: shutdown returned before an already-accepted async reasoner completed/posted terminal status.

NORTH-STAR DELTA:
removed the owner capability blocker without creating a duplicate workspace/runtime; converted #1011 from inferred upstream risk into local executable RED->GREEN evidence; and converted #1000 from a graph finding into a local deterministic RED on the exact owner clone.

STOP CONDITION:
continue the same takeover gate. First make #1000 GREEN with race-safe accepted-work ownership and bounded drain/timeout cancellation semantics; then close #1001 async control-plane persistence/shutdown, #1004 persisted generation/reap, and #1046 stale workflow-vs-execution activity. Do not create the final AgentField commit, run FB-0, or claim the broader F06/F07 family closed until the complete P0 matrix, graph re-audit, final BMAD review, frozen-SWE compatibility, and real restart injection are GREEN.
...
The exact-source route is now blocked one layer higher: **DEV VPS Terminal owner configuration**. Canonical source readback shows `gateway/server-live.mjs` constructs `TargetRegistryACI` from `TARGET_REGISTRY_MUTATION_ALLOWLIST` (default `coding-runtime,coding-api`), and Coolify app `vps-terminal-dev` (`tsnhqqr60rcacdv8kfiw4eqf`, repo `n0namer/vps-terminal`, branch `archops/k4b-container-cleanup`, deployed commit `7ed1daa2824096e2624c0025f7ee9571fa78ff87`) has that environment key configured. The bounded target upsert is already user-authorized but cannot execute until this server-owner allowlist includes `agentfield-dev-workforce`. **Next move requiring new scope authorization:** update only `TARGET_REGISTRY_MUTATION_ALLOWLIST` on that DEV app to add `agentfield-dev-workforce`, restart/redeploy the DEV gateway, verify health/readback, then repeat the already-previewed target registry upsert adding `/tmp/agentfield-f06-fix` while preserving every other target field. Do not edit the runtime registry file directly and do not create a duplicate workspace/runtime. After the typed route is verified, immediately resume AgentField RED->GREEN: first pin #1011 notify-window ordering, then close applicable #1000/#1001/#1004/#1046 boundaries, targeted `-race`, full Go/control-plane/Python-as-changed suites, `git diff --check`, Codebase Index + Graphify re-index, one final BMAD adversarial/edge review, exact local commit, frozen-SWE validation and real restart fault. Only fresh GREEN evidence can unblock **FB-0 full `swe-planner.build` attempt 5**.
