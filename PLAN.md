...
CURRENT BLOCKER:
The active P0 is now **canonical provenance/writeback required by the native SWE runtime**, not product behavior and not another canary harness. Fresh container readback shows the durable AgentField workforce container `workforce-edshqtkwskg3lrczekhcmd71-021517011499` is exited with code `31` after three retries. Its own bootstrap proves the desired runtime identity is already correct: AgentField source reconciles to exact `c0923acdfca043c2c07e3d34daaa09e2a7e41d38`, and SWE is explicitly requested at exact tested commit `6f5b4382e6231721f60be7045b9d91fd85e34fb5`. Startup intentionally aborts because `/opt/reconcile_dev_workspace.sh` requires the desired SWE SHA to equal current canonical `origin/dev`, while `origin/dev` is `3716b28d61de9730af22c459b6252b9ab8f3a8a9` and `6f5b4382...` is still only a tested local commit. Therefore the native lifecycle is correctly enforcing `tested identity = canonical deliverable identity`; bypassing this guard would violate the North Star.

THIS BATCH:
shifted fully to the architecture-owned path requested by the project: no more managed-terminal acceptance canaries. Runtime inventory proved the native workforce owner is down because its provenance guard rejected a non-canonical SWE SHA, not because OpenCode/FCM/AgentField behavior failed. SourceLoop journal independently confirms the exact two-file SWE planning fix was captured as native live-patch events on `/src/swe-af` (`planning.go` and `planning_test.go`) and remains `git_writeback_status=PENDING`; however the target has `writeback={}` so capture cannot yet materialize canonical Git. The mounted runtime-capture service exposes `/src` but its registered `live_patch_roots` correctly exclude `/src/swe-af`, so that service is not used as a write bypass. Registered-target reload of the exited workforce is also correctly blocked because the target does not declare `reload` capability. This narrows the owner-layer gap to canonical SWE writeback/publication plus native desired-SHA rollout, not application debugging.

NORTH-STAR DELTA:
the project is now following the full intended SWE architecture: live patch -> executable validation -> SourceLoop/provenance -> canonical Git identity -> native package/workforce reconciliation -> full `orch.Build` -> independent oracle. The native runtime itself is preventing us from running an uncanonical tested commit, which is the correct safety property. Closing this writeback gap materially shortens the path to the first honest 1/3 full-Build acceptance task.

STOP CONDITION:
do not restart/redeploy the native workforce against a local-only SWE SHA and do not recreate another substitute runtime. First materialize the exact validated two-file SWE delta into canonical `dev` through the canonical source/writeback owner, preserving all later PLAN-only commits; validate the resulting exact canonical source and record its new SHA. Then update the native desired SWE SHA to that canonical commit through the existing workforce/service owner and let the normal durable package lifecycle reconcile itself. Only after the workforce is healthy on exact canonical SWE + AgentField c092 and passes the full Acceptance Runtime Equivalence preflight may FB-0 restart with zero operator intervention.
