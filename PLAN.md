...
Nearest compulsory move: **do not reload/reinstall the active Go node, do not canonicalize the dirty Go tree, and do not claim Telegram/HITL acceptance while executable exact-tree validation and runtime proof are missing.** The smallest high-value route remains a bounded typed Go validation capability on `agentfield-dev-workforce` for the exact `/src/swe-af/go` owner, limited to the canonical gates (`gofmt -l .`, `go build ./...`, `go vet ./...`, `go test -race -count=1 ./...`). That capability-surface change still requires explicit authorization before mutation. Fresh anti-drift confirms the shared-runtime safety gate is still active. CURRENT active-execution visibility remains unavailable (`listActiveAgentfieldExecutions -> Bad Gateway`), so the absence of parallel user work cannot be proven. `/afhome/installed.yaml` still reports the shared `swe-planner` running on port `8800`, PID `389456`, started `2026-09-06T11:10:36Z`; the live `/src/swe-af/go` tree is still broadly dirty, and Coding Station readiness currently returns `Gateway Timeout`. Therefore **do not rebuild/reinstall/restart the shared running `swe-planner` while the user may be running parallel SWE work on this instance.** Runtime lifecycle is a separate release-window gate: first obtain CURRENT proof that no user execution is active, or validate/accept through a separately isolated canary/shadow runtime that cannot affect the shared planner. If active-execution visibility is unavailable, treat that as a safety blocker and keep the running planner untouched. Only after a safe lifecycle window exists may the installed artifact be replaced and one concrete `AskUserForm -> HITL_DECISION_EVENT -> unchanged human pause` execution be accepted. In parallel, close Telegram E2E with one real `/swe <project> ...` inbound on `@claude_code_nikita_bot`. Only after these proofs should Batch 2 adaptive council/deep research and typed Telegram `STEER` begin.
