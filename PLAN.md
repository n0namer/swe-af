...
CURRENT BLOCKER:
The active P0 remains **F16 fail-closed resume on corrupted checkpoint** in SWE exact source. Static owner-source readback proves the defect: `loadCheckpoint()` returns `nil` for both missing/unreadable and malformed JSON; `RunDAG(..., WithResume(true))` treats `nil` as a fresh state and immediately writes a new checkpoint, so a corrupt/partial checkpoint can be misclassified as “no checkpoint” and replay already-completed non-idempotent effects. The prior statement that no lawful Go runner exists was incorrect. The canonical validation environment is the already-proven SWE/workforce container `workforce-edshqtkwskg3lrczekhcmd71-021517011499` / image `edshqtkwskg3lrczekhcmd71_workforce:3134680547875eaf6ee67f4018978ebe56e68833`, mounted with `/src`, `/core-src`, `/build`, `/afhome`, `/workspaces`; this is the same environment where CURRENT Go tests previously passed. It currently exists but is stopped (`Exited 31`, restart_count=3). Therefore the immediate blocker is **validation-environment availability**, not absence of a known Go-capable lane.

THIS BATCH:
followed BMAD test-design/trace plus systematic-debugging and verification-before-completion without touching services or applying a speculative fix. Existing resume tests remain sufficient to prove the RED is non-duplicate: they cover missing checkpoint plus valid resume, not malformed checkpoint. The discriminating RED remains exact: create malformed `artifacts/execution/checkpoint.json`, call `RunDAG(..., WithResume(true))`, require non-nil error before any `ExecuteFn` invocation, and assert the corrupt checkpoint bytes are unchanged. Reuse-first validator search was repeated from fresh CURRENT inventory: `agentfield-dev-runtime-capture` still has no Go in standard locations; registered `workbench` has no `/usr/local/go/bin/go`; Coding Station still returns `Gateway Timeout`; its running coding-runtime has no Go toolchain. A second healthy `workbench-v2` container was discovered and inspected read-only, but direct exec remains blocked by operator mediation because authoritative target scope is unproven, so it was not capability-hopped. No lawful exact-source Go runner is currently callable. No product source was mutated.

NORTH-STAR DELTA:
F16 is now a fully localized source defect with a non-ambiguous acceptance contract, and the only thing preventing RED->GREEN is the validation lane rather than uncertainty about the code. This preserves evidence quality and prevents a blind checkpoint fix from being mistaken for progress.

STOP CONDITION:
do not patch F16 and do not touch runtime/services merely to manufacture a green result. First restore or identify a lawful Go-capable validation lane for the exact `/src/swe-af` snapshot (or an exact immutable snapshot of it). Once executable RED is observed, make the smallest owner-layer fail-closed change, then run targeted DAG/orch tests, full `go test ./... -count=1`, `git diff --check`, and one adversarial/edge review. Until then status is `BLOCKED / VALIDATION_BLOCKER`, not application FAIL.
...
Do **not** spend the next batch on service startup, native package rollout, canonical writeback, or another FB-0 run. The sole next move is **F16 corrupt-checkpoint fail-closed RED->GREEN** on exact SWE source. First obtain a lawful Go runner for the exact source snapshot without mutating services or creating a substitute acceptance runtime. Add one discriminating DAG regression: create malformed `artifacts/execution/checkpoint.json`, call `RunDAG(..., WithResume(true))`, require a non-nil error before any `ExecuteFn` call, and prove the corrupt checkpoint bytes are not overwritten. Only after that RED is observed may implementation change `loadCheckpoint`/resume handling so missing checkpoint and corrupt/unreadable checkpoint are distinct and corrupt resume fails closed. Then run targeted DAG/orch tests, full `go test ./... -count=1`, `git diff --check`, and one adversarial/edge review. If no lawful exact-source Go runner is available, stop `BLOCKED` at `VALIDATION_BLOCKER`; do not patch speculatively and do not capability-hop into runtime/service work.
