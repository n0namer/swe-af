...
Continue the AgentField takeover gate on exact owner clone `/tmp/agentfield-f06-fix`. The applicable P0 behavior matrix (#1000/#1001/#1004/#1011/#1046 plus heartbeat/version fencing and Go cancel-owner ABA) is locally GREEN with fresh storage/handlers/server evidence. **Immediate next move is candidate hygiene and exact identity:** remove/exclude only the unsupported Python ABA experiment and generated `.archsteer/`, preserving every proven Go/control-plane change; rerun focused Go SDK/control-plane validators plus `git diff --check`; then create one local-only AgentField commit from exactly that tested delta. The typed Python rollback is currently blocked by operator `required_context:context_check_unavailable`, so do not bypass it with opaque shell mutation. Once a clean exact candidate exists, validate frozen SWE via local module replace/full Go suite, rebuild the planner from that exact AgentField commit, and repeat the same F06 SIGTERM/restart fault injection. Re-index with Codebase Index/Graphify if the mediated lanes become callable; otherwise record the capability gap and do not block exact executable evidence. Resume **FB-0** only when tested AgentField identity = exercised restart identity and the restart oracle remains GREEN.
