...
Nearest compulsory move: **do not reload/reinstall the active Go node, do not canonicalize the current Go runtime tree, and do not enable AUTO/council while exact-tree executable validation is unavailable.** First restore an existing validation route for the same CURRENT `/src/swe-af/go` tree: preferred path is repair/recover Coding Station writable workspace or enable a bounded typed Go validation preset on `agentfield-dev-workforce` (this is a capability-surface change and needs explicit authorization before mutation). Then run whole-tree canonical Go gates (`gofmt -l .`, `go build ./...`, `go vet ./...`, `go test -race -count=1 ./...`) on the exact CURRENT source snapshot. Only after PASS may the local `swe-planner` package be rebuilt/reinstalled and runtime-accepted with `AskUserForm -> HITL_DECISION_EVENT -> unchanged human pause` on the same execution. In parallel, Telegram project-qualified Mission Control remains active and awaits one real user-originated `/swe <project> ...` acceptance. Batch 2 adaptive council/deep research and typed Telegram `STEER` start only after these gates.
