...
CURRENT BLOCKER:
The active AgentField P0 takeover matrix is now **behaviorally GREEN on the exact owner clone**: #1011 notify/admission ordering, #1000 graceful drain/settlement, #1001 async admit-before-persist + pool shutdown terminalization, heartbeat/version compatibility, #1004 persisted generation ownership/deferred instance-scoped reap, and #1046 stale workflow protection. #1046 has deterministic GREEN for both required invariants: recent paired execution activity protects an old workflow, and a heartbeat arriving after candidate selection prevents the final timeout UPDATE. Fresh full `internal/storage -count=1` PASS; fresh `internal/handlers` + `internal/server -count=1` PASS; full `sdk/go ./... -count=1` PASS; exact-clone `git diff --check` PASS. The proven Go/control-plane candidate is 26 tracked files plus canonical migration `035_execution_instance_id.sql` (current diff stat: 1,148 insertions / 133 deletions); the three Python ABA files remain unsupported experiment-only dirt and `.archsteer/` remains generated output, so neither belongs in the candidate. The remaining blocker is now **canonicalization capability**, not product behavior: typed live patch/test works, but operator mediation rejects both scoped `git restore` and exact-file `git add` as `OPAQUE_GIT_MUTATION`; SourceLoop has 309 captured live-patch events but a fresh capture artifact lookup returns `capture_artifact_reference_invalid`, so there is no CURRENT typed artifact/commit route. Do not bypass with GitHub programming or another workspace. Race evidence is additionally environment-blocked: normal `-race` crashes in legacy `boltdb/bolt@v1.3.1` checkptr before the test; disabling checkptr preserves `-race` but exceeds the 120s validator ceiling without verdict.
