...
CURRENT BLOCKER:
The active AgentField P0 takeover matrix is now **behaviorally GREEN on the exact owner clone**: #1011 notify/admission ordering, #1000 graceful drain/settlement, #1001 async admit-before-persist + pool shutdown terminalization, heartbeat/version compatibility, #1004 persisted generation ownership/deferred instance-scoped reap, and #1046 stale workflow protection. #1046 has deterministic GREEN for both required invariants: recent paired execution activity protects an old workflow, and a heartbeat arriving after candidate selection prevents the final timeout UPDATE. Fresh full `internal/storage -count=1` PASS; fresh `internal/handlers` + `internal/server -count=1` PASS; full `sdk/go ./... -count=1` PASS; exact-clone `git diff --check` PASS. The user has explicitly authorized the narrow persistent/canonicalization work required to materialize the tested candidate. Canonical repository `n0namer/agentfield` contains base `4d337c1ae5104418311fcba414a1c2f85c2abb89`; local-only `b160245833ec51f5296905c32e49260a62c76e26` contributes the earlier proven 5-file Go SDK F06 layer. An exact writeback artifact was generated from canonical `4d337c1...` to the CURRENT tested Go/control-plane state, excluding all Python experiment files and `.archsteer/`: `/tmp/agentfield-f06-fix/.candidate-writeback.patch`, 102801 bytes / 2382 lines, SHA256 `dad823bc9622e19a4004529d694d892e283d6a240048d1ce758d83638a8066a3`. Coding Station repository `agentfield` accepted a clean session on exact base `4d337c1...`, but its API is currently returning `Gateway Timeout`; an attempted streaming apply timed out before stdin completion and independent `git status --short` proved the canonical session remained clean. Therefore the remaining blocker is **temporary writeback-lane availability**, not scope, provenance or product behavior. Do not bypass with GitHub-edit programming. Race evidence is additionally environment-blocked: normal `-race` crashes in legacy `boltdb/bolt@v1.3.1` checkptr before the test; disabling checkptr preserves `-race` but exceeds the 120s validator ceiling without verdict.

THIS BATCH:
used BMAD test-design/quick-dev/adversarial/trace plus systematic-debugging/TDD/verification-before-completion. Closed #1046 at the storage owner boundary rather than with a handler workaround: workflow candidate selection now consults the paired non-terminal execution activity clock, and the final conditional UPDATE re-checks workflow freshness, pending approval, active children and paired execution freshness to close the SELECT->UPDATE race; the legacy execution stale reaper also re-checks freshness/children at UPDATE. Both discriminating #1046 tests GREEN. Full storage then PASS; handlers/server PASS; `git diff --check` PASS. Final adversarial inventory confirmed the Python cancellation-owner delta remains unsupported experiment-only work, so it is intentionally excluded from the candidate; an attempted scoped rollback was blocked before mutation by Required Context degradation, leaving source unchanged.

NORTH-STAR DELTA:
all currently applicable restart/generation/shutdown/stale-state P0 behaviors discovered in the takeover graph are now executable local contracts on the frozen AgentField lineage. The project is one hygiene/exact-identity step away from creating the single tested local AgentField candidate for frozen-SWE and real restart validation.

STOP CONDITION:
first obtain a clean candidate boundary: revert/exclude the three unsupported Python experiment files and generated `.archsteer/` without touching proven Go/control-plane changes; rerun the relevant exact-source suites and hygiene check; then create one local-only exact AgentField commit, validate frozen SWE through local module replacement, and repeat the real restart fault injection with tested identity = exercised identity. Only after that final evidence gate may FB-0 resume.
...
Continue the AgentField takeover gate on exact owner clone `/tmp/agentfield-f06-fix`. The applicable P0 behavior matrix (#1000/#1001/#1004/#1011/#1046 plus heartbeat/version fencing and Go cancel-owner ABA) is locally GREEN with fresh full Go SDK/storage/handlers/server evidence and `git diff --check` PASS. **Immediate next move is owner-side canonicalization capability, not more product coding.** Preserve the tested 26-file Go/control-plane delta + migration exactly; exclude the three unsupported Python experiment files and `.archsteer/`. Current DEV mediation blocks local Git staging/restore as `OPAQUE_GIT_MUTATION`, and SourceLoop captures are journaled but not materialized as valid writeback artifacts. Restore a supported typed commit/writeback route for this existing exact-source target (or materialize its current SourceLoop captures) without creating another workspace and without GitHub-edit programming. Then create one local-only exact AgentField commit, rerun the focused validators on that exact identity, validate frozen SWE via local module replace/full Go suite, rebuild the planner from the exact AgentField commit, and repeat the same F06 SIGTERM/restart fault injection. Resume **FB-0** only when tested AgentField identity = exercised restart identity and the restart oracle remains GREEN.
