...
Nearest compulsory move after this correction: **freeze shared-runtime mutation** and re-establish the lifecycle safety gate first (CURRENT proof that no user execution is active, or an isolated canary/shadow runtime explicitly approved for this purpose). Once safe, rerun the unchanged fixture with no new SWE edits. Only a clean `approved=false`, `blocking=true` verdict may advance to verifier → full `implement_issue` repair → independent oracle.
