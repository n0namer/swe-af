...
Current 30-minute Pareto batch: (1) finish one shared OpenCode permission overlay for coder/reviewer/verifier that blocks observed package-install commands and sibling-worktree venv execution while preserving normal repository-local Bash/tests; (2) add deterministic role tests for the exact overlay; (3) run Python compile + targeted Go role/prompt tests + `go vet ./...` + `go test ./...`; (4) write back exact evidence before another long L3 run. In parallel, use BMAD TEA-style test design and trajectory-eval practice to add a tiny deterministic micro-probe later that records attempted-vs-executed policy violations; do not introduce a new eval framework until existing logs/tests prove insufficient.

DoD for this batch: shared enforcement owner exists; coder/reviewer/verifier all receive the same current-worktree-safe permission contract; exact source gates pass; PLAN/readback names the tested delta; no product redeploy or GitHub-code programming is used. L3 remains OPEN until a fresh same-case run reaches repair -> verifier -> delivery and independent operator acceptance on the delivered SHA.

One next move: complete and validate the runtime permission overlay in live `/src/swe-af`, then rebuild the process-only planner from that exact source and run one short policy micro-probe before spending another full L3 trajectory.
