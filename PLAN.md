...
CURRENT BLOCKER:
The active P0 is **runtime materialization equivalence before the next countable FB-0 run**, not another SWE planning/schema fix. AgentField takeover remains VERIFIED/CLOSED at `c0923acdfca043c2c07e3d34daaa09e2a7e41d38`; repaired frozen SWE remains `6f5b4382e6231721f60be7045b9d91fd85e34fb5`; exact planner binary remains `/tmp/swe-planner-fb0-6f5b438-c092` SHA256 `3dc66b5d7f727f1b3e973e8e2fe6b7b29004853d7079f26fa599e13bacab5cd3`. Attempts 5-7 are diagnostic/non-counting evidence. Attempt 8 finally used the native OpenCode wrapper and progressed through real FCM/model execution plus successful git-init, but its canary planner and c092 control-plane were still owned by managed terminal sessions; fresh readback now shows both gone and `127.0.0.1:18085` unreachable, so terminal state is not independently recoverable. Attempt 8 is therefore `EVIDENCE_MISSING` for acceptance, not task 1/3. A durable native `swe-planner` process still exists at `/afhome/packages/swe-planner/bin/swe-planner` (PID observed `643190`) under the normal package/runtime contract, including `SWE_OPENCODE_BIN=/afhome/bin/opencode`, `SWE_DEFAULT_RUNTIME=open_code`, and `SWE_DEFAULT_MODEL=fcm/fcm`.

THIS BATCH:
identified and corrected the experiment-design mistake: diagnostic process-only runtimes were allowed too close to acceptance. Added an explicit Acceptance Runtime Equivalence Gate and preflight rule to SoT. Root cause of Attempts 6/7 was not one thing: Attempt 6 found a real PM OpenCode overlay parity defect and legitimately produced the bounded framework fix `6f5b4382...`; Attempt 7 additionally showed that manually running the planner inside the AgentField workforce omitted the native OpenCode executable/bootstrap contract; Attempt 8 repaired dependency/bootstrap equivalence by reusing `/afhome/bin/opencode`, and real model work/git-init progressed, but lifecycle equivalence remained false because the canary control-plane/planner were terminal-session-owned and disappeared before durable terminal readback. This means further Build attempts on ad-hoc sessions would optimize the test harness rather than advance autonomous engineering evidence.

NORTH-STAR DELTA:
the project now distinguishes **diagnostic proof** from **autonomy acceptance proof**. North Star requires a real engineering task on the normal durable SWE runtime lifecycle, not merely equivalent binaries launched manually. This prevents false progress and moves the next effort to the last missing prerequisite for meaningful 3/3 evidence: exact tested source running under the native durable package/service lifecycle with independently persistent execution state.

STOP CONDITION:
do not start another countable FB-0 Build from a managed terminal/ad-hoc localhost control-plane. Reuse the existing native SWE package/runtime owner and materialize the exact `6f5b4382... + c092` candidate through its normal package/service lifecycle, or prove an existing native generation already runs that exact identity. Before START, pass the full Acceptance Runtime Equivalence preflight including post-session durability. Then run FB-0 with zero operator intervention. If a native owner route cannot materialize the exact identity, stop at `RUNTIME_MATERIALIZATION_GAP`; do not substitute another cut-down runtime.
...
Do **not** start another FB-0 attempt from a managed terminal canary. The next 30-minute batch must materialize the already-tested exact candidate through the **existing durable native SWE package/runtime lifecycle** that owns `/afhome/packages/swe-planner/bin/swe-planner` and `/afhome/bin/opencode`, while preserving AgentField `c0923acdfca043c2c07e3d34daaa09e2a7e41d38` and frozen SWE `6f5b4382e6231721f60be7045b9d91fd85e34fb5`. Reuse the native owner/update mechanism rather than copying a binary and supervising it from an operator session. DoD before the next countable START: (1) runtime-reported exact source/planner/AgentField identity matches the tested candidate; (2) real native OpenCode wrapper `fcm/fcm` canary PASS with model progress; (3) callback/control-plane registration is reachable; (4) initiating operator session can end and the planner/control-plane/execution readback remains alive and independently queryable; (5) clean target worktree and zero pre-existing live mutator. Only after all five are GREEN may the next FB-0 Build start under the frozen config and intervention-free acceptance rules. If the native package/update owner cannot load exact `6f5b4382... + c092`, record `RUNTIME_MATERIALIZATION_GAP` as the sole blocker; do not create another substitute runtime.
