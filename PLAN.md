...
CURRENT BLOCKER:
The active P0 is **SWE code convergence / behavior completeness on exact source**, not canonical writeback, service rollout, or another acceptance run. The latest full-Build attempts proved that the code/runtime contract is still incomplete: Attempt 6 exposed a real Product Manager OpenCode/FCM parity defect; Attempt 7/8 then exposed runtime-materialization/lifecycle gaps that made acceptance evidence invalid. These are useful diagnostics, but they do not prove that the SWE codebase is ready for deployment. Until the remaining North-Star P0 contracts are executable-green on exact source, canonicalization and service lifecycle work are downstream and must not consume the critical path.

THIS BATCH:
corrected the phase ordering. The project had prematurely moved from implementation/debugging into provenance/runtime rollout. The new order is explicit: (1) exact-source code convergence, (2) deterministic validation of all currently proven P0 behavior gaps, (3) adversarial/edge review and one exact tested candidate, (4) only then canonical Git/SourceLoop publication, (5) native durable runtime materialization, (6) autonomous full `orch.Build` acceptance. Existing runtime/provenance findings are retained as downstream constraints, not as the current blocker.

NORTH-STAR DELTA:
this prevents infrastructure work from substituting for unfinished product engineering. The fastest route to the North Star is now to finish the code first: every 30-minute batch must either close a proven SWE behavior gap with RED->GREEN evidence or prove that no such gap remains and advance the code-complete gate. Runtime/service work resumes only after that gate is satisfied.

STOP CONDITION:
do not restart/redeploy/materialize services merely to obtain more acceptance evidence while known SWE code gaps remain. Close the code-complete gate first: enumerate the currently proven P0 SWE behavior gaps from existing evidence, resolve them on the exact source with discriminating tests, run targeted + full canonical validation, perform one adversarial/edge review, and produce one exact clean tested candidate. Only when that candidate is behavior-complete for the active North-Star path may the project proceed to canonical writeback and native runtime/E2E.
