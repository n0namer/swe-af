...
- OpenClaw reuses the default paired Telegram/Jarvis path. New canonical workspace skill: `/home/node/.openclaw/workspace/skills/swe-governor/SKILL.md` plus deterministic adapter `scripts/swe-governor.mjs`; OpenClaw reports the skill eligible/model-visible/user-invocable. Adapter `poll` reads only `http://workforce:8800/agentfield/v1/logs`, tracks pending/resolved request ids in a 0600 local state file, and returns `NO_REPLY` when no decision exists. Mutating commands validate the pending id/type and POST only to the existing `http://workforce:8800/webhooks/approval` callback.
- OpenClaw cron declaration `swe-governor-decisions-v1`, job id `c7ea3f43-2a02-4752-aaac-f254f9ba256f`, is enabled every **20s** as a command job with no LLM. A reversible manual canary temporarily replaced the command payload while the schedule was disabled; the run completed `status=ok`, `delivered=true`, `deliveryStatus=delivered` to the existing paired Telegram operator, after which the original `swe-governor poll` command was restored and the job re-enabled. Post-restore CURRENT readback: `everyMs=20000`, last run `ok`, ~127ms, diagnostic `NO_REPLY`, `lastDelivered=false`, errors/skips `0`, proving quiet no-pending behavior. No second bot, daemon, HAX deployment, or new messaging database was created.
...
- Live end-to-end pause/resume canary is **BLOCKED by provider before HITL**, not by Governor. Two bounded `run_product_manager` canaries (`exec_20260905_110441_k9swye3b`, `exec_20260905_110556_0ibik04v`) failed before producing an `ask_user_form` with `API error: rate limit exceeded: too many concurrent requests`; no third identical retry is permitted without new evidence. Therefore source/transport/cron contracts are PASS, but `waiting -> Telegram -> decision -> resume` remains L2 PENDING until a real SWE execution reaches the pause seam.
