...
- Current clean precondition: sacrificial repo `/tmp/swe-af-fullbuild-current-20260912` is clean on baseline branch `fullbuild-current-baseline-20260912`, exact local SHA `cdc39105e92937e0085410a656346471b2dc6834`; full target Go suite PASS; planner `/tmp/swe-planner-f06-sdk-20260913` (SHA256 `581862f59722400ee0999176339dfb99f8dc67cd7901bf39d9861caa5df4d4`) is healthy/registered on PID `548849` with a non-empty process `instance_id`. Require zero live OpenCode process targeting that workspace immediately before attempt 5 START.
...
CURRENT BLOCKER:
AgentField #1011, #1000 and the active-path #1001/heartbeat boundaries are now **GREEN on the exact owner clone**. #1011 has deterministic notify-window RED->GREEN plus targeted `-race` and full Go SDK PASS. #1000 has natural-drain RED->GREEN, bounded timeout cancellation/terminal settlement, targeted `-race` and full Go SDK PASS. #1001 now reserves concurrency and queue capacity before persistence, rejects failed admission with zero execution/workflow state, owns a native stoppable async worker pool that terminalizes accepted running/queued work, and is wired into real `AgentFieldServer.Stop()`; after repairing package-global test isolation, the full `control-plane/internal/handlers` suite PASS. Heartbeat fencing now preserves native versioned-agent/canary lookup while explicitly covering current modern, legacy-empty, stale mismatch and missing-after-modern instance semantics. The active P0 is #1004 persisted generation ownership. Its first storage oracle is a genuine RED: neither `executions` nor `workflow_executions` currently exposes the required `instance_id` column. Implementing the canonical migration is presently blocked because typed `fileAction(create)` for the exact clone is denied as a target-capability mismatch even though stale-safe existing-file patch/test access is working.

THIS BATCH:
kept the user-mandated native-architecture-maximization rule: no local in-memory substitute was accepted where AgentField already owns a native lifecycle/persistence primitive. Closed #1000 through SDK accepted-work ownership + graceful drain/timeout settlement; closed #1001 through native pre-persistence admission reservations, worker-pool lifecycle and server shutdown integration; isolated the suite-only async failure to package-global test state and made the success test self-contained without weakening production semantics; restored versioned heartbeat lookup and pinned all four compatibility/fencing states; final full `internal/handlers -count=1` PASS. Then moved into #1004 at the persistence layer and obtained deterministic schema RED via SQLite `PRAGMA table_info`. An attempted creation of canonical `migrations/035_execution_instance_id.sql` was denied by the operator before filesystem mutation with `DENY: out_of_scope` / `target-registry-capability-mismatch`; no shell/GitHub-edit bypass was used.

NORTH-STAR DELTA:
three previously graph-only restart/shutdown risks are now executable local contracts using the native AgentField architecture, and versioned/canary heartbeat functionality was preserved rather than sacrificed for restart fencing. The remaining generation problem is now proven at the real persistence boundary instead of being represented by a narrower handler workaround.

STOP CONDITION:
continue the same takeover gate. First restore a supported typed **canonical new-file creation** route for the already-authorized exact AgentField clone, then create migration `035_execution_instance_id.sql` and close #1004 end-to-end: native type/model fields, SQL read/write propagation, serving-instance stamping, instance-scoped reap including explicit legacy rows, and deferred drain-grace cleanup on re-registration. Then close #1046 paired execution/workflow activity semantics. Do not capability-hop, do not GitHub-edit AgentField as a debug loop, do not create a duplicate workspace/runtime, and do not start FB-0 before the complete P0 matrix and final evidence gate are GREEN.
...
Continue the AgentField takeover gate on exact owner clone `/tmp/agentfield-f06-fix`. #1000, #1011, #1001 and heartbeat/versioned-instance compatibility are locally GREEN; the active P0 is #1004 persisted generation ownership. The first storage oracle is RED because `executions` / `workflow_executions` lack native `instance_id`. **Immediate owner-layer blocker:** the required canonical migration is a genuinely new file, while DEV `fileAction(create)` maps to `TARGET_CANARY_CREATE` and requires target capability `canary_file`; `agentfield-dev-workforce` currently declares `live_patch` but not `canary_file`, so mediation correctly denied creation. The next state-changing move is a narrow persistent target capability change: add only `canary_file` to the existing `agentfield-dev-workforce` capability set, preserving selector, roots (`/src`, `/tmp/agentfield-f06-fix`), Python live-patch runtime, checks and all other fields; independently read back the registry before using it. Then create only canonical `control-plane/migrations/035_execution_instance_id.sql`, run the schema RED->GREEN, and complete #1004 through native types/models, SQL persistence, serving-instance stamping, instance-scoped departing-generation+legacy reap and deferred drain-grace cleanup. After #1004, close #1046 paired execution/workflow activity, rerun race/full suites, re-index exact source with Codebase Index + Graphify when callable, run final BMAD adversarial/edge review, create one exact local tested AgentField commit, validate against frozen SWE and repeat the real restart fault injection. Do not capability-hop, create another workspace/runtime, or start **FB-0** before all applicable P0 boundaries are GREEN with tested identity = exercised identity.
