...
### Telegram Mission Control routing hardening (2026-09-07)

CURRENT inbound evidence exposed a UX/control-plane gap: fresh dedicated-SWE cards are mapped correctly (`messageId 373 -> swe-af`, `374 -> openclaw`), and Telegram did deliver an inbound message to `@claude_code_nikita_bot` at 2026-09-07 09:28 local, but the transcript body was literal `Reply` with **no Telegram reply metadata**. The Governor therefore could not bind it to either card; OpenClaw correctly fell through to the ordinary devteam model. Depending exclusively on Telegram UI Reply is too brittle for the Mission Control North Star.

The live `swe-governor-telegram` control surface has therefore been hardened without recency inference: ReplyToId remains highest-authority routing; when ReplyToId is absent, `/swe <project> <text>` or `<project>: <text>` may resolve **only when exactly one pending request exists for that exact `payload.project_id`**. Zero matches fail closed (`NOT_SWE_PROJECT` / explicit warning); multiple pending requests for one project fail closed and require a specific card/request. Non-terminal project-qualified text is persisted to that exact Governor discussion thread before ordinary model conversation; explicit terminal intents still use the existing exact request action bridge. Adapter command `route-project` implements the exact-project lookup. Parser tests PASS `4/4`; `node --check` passes for both plugin and Governor adapter; current `route-project swe-af` and `route-project openclaw` map exactly to canaries `canary-live-sweaf-20260907` and `canary-live-openclaw-20260907`; unknown project returns no route; a temporary two-pending same-project canary produced `AMBIGUOUS_SWE_PROJECT 2` and was removed, proving fail-closed ambiguity handling. OpenClaw was reloaded in-process with native `SIGUSR1`; health returned and `openclaw plugins inspect swe-governor-telegram` reports `Status: loaded`. CURRENT hashes: parser `1ea9d0b5c118339c309a8b7f01b5dbe14da2a38fbfa5b0e3497c69f8a3145833`; plugin `09d5195d6108603828b2bdc48422543e57e07b48d0d3fa43cb993b7aeb7a7b14`; Governor adapter `5b40e0e7dfc69f51487500686bea86dd08e4794704e73a1be0d72b836684e408`; parser tests `a2158534725e38debd8e38eb1361689210bc538eda437c9d5861f52a93033bef`. A **real user-originated** `/swe <project> ...` inbound has not yet been observed, so this routing mode is not yet end-to-end accepted.

Go Shadow Decision Run static API audit also completed while executable validation remains blocked: CURRENT `harnessx.RoleOptions` contains every field used by the new resolver (`Provider`, `Model`, `MaxTurns`, `Tools`, `PermissionMode`, `SystemPrompt`, `Cwd`); `harnessx.Run[T]` returns `(*T, *harness.Result, error)` and `result.Parsed` is the same validity signal already used by existing roles. Required imports/types are present, and the HAX/OpenClaw-disabled branch remains before the shadow pass, so existing disabled-HITL semantics are unchanged. This is compatibility evidence only, not a substitute for executable Go tests.

### Exact-tree Go validation anti-drift (2026-09-07, later batch)

Fresh anti-drift 2026-09-07 14:xx supersedes the earlier Coding Station availability wording while preserving its ENOSPC evidence. `n0namer/swe-af@dev` HEAD remains `9b3c449902238b96a70a90f4fc28c510c35bd00c`; root `PLAN.md` is still the canonical Project SoT. `agentfield-dev-workforce` still owns CURRENT source `/src/swe-af/go`, and `/afhome/installed.yaml` still reports the separately installed `swe-planner` package running on port `8800`, PID `389456`, started `2026-09-06T11:10:36Z`; code-on-disk therefore still does not prove loaded runtime identity. CURRENT `git status --short` still shows the broad dirty runtime tree, including the Decision Run files plus many unrelated concurrent Go changes. Telegram Mission Control canaries `373` (`swe-af`) and `374` (`openclaw`) remain `pending` with empty `discussion`, so no real inbound steering/HITL acceptance has happened yet.

Coding Station changed state since the previous note: its public action API now returns `Gateway Timeout` for both readiness and health calls. Read-only Docker evidence shows the existing `coding-api` and `coding-runtime` containers themselves are still `running`, Docker-health `healthy`, restart_count `0`, and their only recent logs are normal listen events. The runtime owns separate `/data/workspaces`, `/data/repos`, `/data/cache`, `/data/artifacts`, `/data/project-*` volumes, while the API owns its own station-state volume. Direct container exec into that helper runtime is operator-mediation `OUT_OF_SCOPE`, so no helper restart/redeploy or volume mutation was attempted. The earlier two exact-base repo sessions remain valid evidence that writes failed on the first patch with `ENOSPC` despite free blocks/inodes; no validation delta was partially applied and both sessions were discarded.

More importantly, CURRENT `/src/swe-af/go` is not a six-file Decision Run delta. `git status --short go` shows 37 modified Go/runtime files across HITL, harness, DAG, orchestration, coding, prompts, schemas, node registration and package metadata. `git diff --stat -- go` reports **1725 insertions / 329 deletions**; whole-tree `git diff --check -- go` PASS. SourceLoop reports 89 matching live-patch journal entries for `agentfield-dev-workforce`; current Go Decision Run entries are `PENDING`, and recent entries have `capture_envelope=null`, so there is no ready exact-source bundle to hand to another runner. Reinstalling `/src/swe-af/go` after testing only the Decision Run files would therefore ship unrelated unvalidated live changes and is prohibited.

Existing-container reuse was checked before declaring a capability gap. `runtime-capture` shares the exact `/src` volume but has no Go toolchain. The Coding Station runtime has Go but mounts its own `/data/...` volumes, not the SWE `/src` volume, and its repo workspace is currently write-broken. No other currently observed container provides both the SWE runtime-source volume and an evidenced Go toolchain. Therefore exact-tree executable validation currently requires either restoring the existing Coding Station write path or an explicit operator capability change that exposes a typed Go check for the existing `agentfield-dev-workforce` source owner. Do not create a new builder/container and do not copy the 37-file tree into GitHub as a debugging substitute.

### Fresh anti-drift readback (2026-09-07, current batch)

Repository authority is unchanged: `n0namer/swe-af@dev` remains the project repository/branch authority; root `PLAN.md` remains the canonical SoT and no parallel ledger is needed. Exact branch HEAD is intentionally treated as execution metadata rather than a durable design fact because each verified SoT write advances it. The active repository contract explicitly states that root `ERRORS.md` does **not** exist on `dev`, so no new error ledger was created. `/src/swe-af/go` is still the CURRENT runtime source owner and remains broadly dirty across the same multi-file Go/runtime surface; the running `swe-planner` is still a separate installed package with `source_path=/src/swe-af/go`, port `8800`, PID `389456`, started `2026-09-06T11:10:36Z`. Code-on-disk therefore still does not prove loaded runtime identity.

Runtime evidence was refreshed instead of inferred. `/afhome/logs/swe-planner.log` contains no `HITL_DECISION_EVENT`, `hitl_decision`, or `ask_user` entries. Its latest concrete reasoner evidence is still the pre-Decision-Run `run_code_reviewer` execution from 2026-09-06, which failed after a 300s no-progress timeout; the log then records the old node shutting down. This means there is still **no runtime proof** that the current running binary has loaded the new Shadow Decision Run. `agentfield-dev-workforce` target logs continue to show only provenance-helper HTTP traffic, which the repository contract explicitly says is not `swe-planner` readiness evidence.

Validation-plane anti-drift changed again in the 2026-09-07 15:xx batch. Coding Station `stationReady` and `stationHealth` now both return healthy/ready, with Go present in the managed toolchain allowlist, so the earlier statement that the public API is simply unavailable is stale. However `createRepoSession` on exact current `dev` base `e589b3bbd6719aa448b2424deb1929e966682e6e` still times out. A cleanup pass removed three closed/ghost repo-session records (including one session created by a timed-out call), and a post-cleanup retry still returned `Gateway Timeout`, so repo-session creation remains operationally unusable. CURRENT Docker evidence still shows **two** simultaneously running/healthy `coding-api` containers for the same compose service/image and one `coding-runtime`; the registered `coding-api` selector therefore remains ambiguous and target-scoped reads fail with `Expected one permitted target, found 2`. This is proven helper topology drift, but causality for repo-session timeouts or the earlier `ENOSPC` remains unproven; no helper container was stopped/restarted/removed. Telegram Mission Control canaries `373` (`swe-af`) and `374` (`openclaw`) remain `pending` with empty `discussion`; status-only snapshot message `378` remains the latest awareness proof, not inbound steering acceptance. No additional OpenClaw runtime code was changed because the workspace contract still requires an `openclaw-dev-team` Implementation Package for new OpenClaw mutations.

### Fresh anti-drift readback (2026-09-07 16:23 local)

CURRENT authority and runtime facts are unchanged where it matters. `n0namer/swe-af@dev` is still the canonical repository/branch and `PLAN.md` remains the Project SoT. The target registry SHA is `550b785f0a1db7ec8d087f088e9dc5939fca72500e62a1d1be781568150e1db2`; `agentfield-dev-workforce` still exposes `terminal/sessions/logs/process/network/stats/live_patch` with `/src` as workspace/live-patch root, but its checks remain **only** `git_diff_check` — no Go compile/test preset has appeared. `/afhome/installed.yaml` still reports the separately installed `swe-planner` running from local source metadata `/src/swe-af/go`, port `8800`, PID `389456`, started `2026-09-06T11:10:36Z`; the installed package is still not proven identical to CURRENT source.

The exact CURRENT Go source remains broad and unchanged in scope: `git status --short go` lists 37 modified files and `git diff --stat -- go` remains **1725 insertions / 329 deletions**. `/afhome/logs/swe-planner.log` still contains no `HITL_DECISION_EVENT`; therefore Shadow Decision Run is still source-level work without runtime acceptance. Coding Station is not a reliable fallback in this batch: its readiness call again returns `Gateway Timeout`, so no repo-session retry was attempted after that fresh failure. Telegram Mission Control canaries remain exactly `373 -> swe-af` and `374 -> openclaw`, both `pending` with empty `discussion`; no real user-originated steering/HITL acceptance has occurred.

Nearest compulsory move: **do not reload/reinstall the active Go node, do not canonicalize the dirty Go tree, and do not claim Telegram/HITL acceptance while executable exact-tree validation and runtime proof are missing.** The smallest high-value route remains a bounded typed Go validation capability on `agentfield-dev-workforce` for the exact `/src/swe-af/go` owner, limited to the canonical gates (`gofmt -l .`, `go build ./...`, `go vet ./...`, `go test -race -count=1 ./...`). That capability-surface change still requires explicit authorization before mutation. Fresh anti-drift at 2026-09-07 17:34 local confirms the shared-runtime safety gate is still active and adds a control-plane degradation. AgentField gateway health itself is `healthy`, but `listActiveAgentfieldExecutions` still returns `Bad Gateway`, so the absence of parallel user work cannot be proven. Coding Station readiness again returns `Gateway Timeout`. The DEV VPS terminal/control plane is also currently unavailable (`no available server`), so no live source mutation, target-registry mutation, reload, or validation command was attempted through it in this batch. Independent host-level readback still shows exactly three Coding Station containers: two simultaneously running/healthy `coding-api` containers for compose project `e2ya9wowlzmw6cn0a1gbut6h` and one running/healthy `coding-runtime`; the duplicate API topology drift therefore still exists. This host evidence does not authorize helper cleanup and does not prove the duplicate API caused the higher-level timeouts.

Therefore **do not rebuild/reinstall/restart the shared running `swe-planner` while the user may be running parallel SWE work on this instance.** Runtime lifecycle remains a separate release-window gate: first obtain CURRENT proof that no user execution is active, or validate/accept through a separately isolated canary/shadow runtime that cannot affect the shared planner. If active-execution visibility or the DEV operator plane is unavailable, treat that as a safety blocker and keep the running planner untouched. No GitHub→redeploy coding loop is permitted; code continues to be container-first once the live control plane returns. In parallel, close Telegram E2E with one real `/swe <project> ...` inbound on `@claude_code_nikita_bot`. Only after these proofs should Batch 2 adaptive council/deep research and typed Telegram `STEER` begin.

2026-09-08 anti-drift correction: the live operator plane is reachable again, but this session performed a process-only `swe-planner` reload before rereading the freshest lifecycle guard above. That is a **DESIGN_RUNTIME_DRIFT** and must not be repeated. CURRENT post-state is PID `12348`, loaded/installed binary SHA256 `ce9848830cb62fd2343fe610e344255706d514c45453b15466dbc7a8f0087fd0`, callback `http://workforce:8800`, and explicit `SWE_OPENCODE_BIN=/afhome/bin/opencode`; `/src/swe-af` remains on Git baseline `58c4e0d19081bc52363c120b7963a34cebb1e894` with live edits in `go/internal/roles/coding/coding.go` and `coding_test.go`. The reload corrected a real deployment drift where `SWE_OPENCODE_BIN` was absent and `opencode` was not in PATH, causing reviewer runs to fail immediately with `The output file was NOT created`. A reviewer-only live delta also prefers `<worktree>/.venv/bin` in PATH when present; targeted `gofmt`, `go test ./internal/roles/coding ./internal/prompts/coding`, `go vet ...`, and build passed before load. Do **not** canonicalize or reload this delta until the lifecycle safety gate is independently satisfied.

The recreated EvalGuard #3 known-bad fixture is commit `9c6234f4979d310fd04e7b04418c006a04c51f22` on original benchmark base `631cf297dd449bdd08112607818d3f154f078284`; its own mutation tests are `5 passed`, while the hidden `#`-inside-string discriminator returns zero mutations, so the fixture preserves the intended semantic blocker. CURRENT FCM at `/health` is reachable (`HTTP 200`, v0.5.81, `activeSet=fast-coding`, 33 active models, 8 effective providers), but probe health is still degraded: 6 OK / 23 broken across 29 visible probe entries at the 2026-09-08 check. Reviewer execution `exec_20260908_162237_tvck5jup` reached OpenCode/FCM correctly but produced no tool events during the prolonged first main inference and was cancelled as provider-contaminated. Thus clean reviewer termination acceptance remains unproven; do not patch SWE further from that run.

2026-09-08 resumed certification evidence: FCM is materially healthier than the prior degraded window. Five consecutive real `/chat/completions` calls through the configured broker all returned HTTP 200 via `gemini-3.5-flash-lite`, with 0.43–1.31 s latency and exact `OK` responses. On the unchanged known-bad EvalGuard fixture, reviewer execution `exec_20260908_163252_b4d3mnjm` completed cleanly in 123811 ms with `approved=false`, `blocking=true`; it independently ran the intended `#`-inside-literal discriminator, observed `mutations: 0 / NO MUTATIONS`, and correctly concluded that executable semantics are not preserved. This closes the reviewer semantic-detection gate for the fixture on CURRENT FCM/runtime.

Verifier execution `exec_20260908_163604_7lde4s32` then exposed a separate `REPO_ENVIRONMENT` defect: verifier did not prefer the repo-local `.venv/bin`, so `python` was absent, `python3 -m pytest` lacked pytest, and direct imports lacked pydantic. The run was cancelled once this was causal. Live source now applies the same OpenCode-only repo-local virtualenv PATH preference to `RunVerifier`; `gofmt`, `go test ./internal/roles/advisor ./internal/roles/coding ./internal/prompts/coding`, and `go vet` PASS. This verifier delta is source-only and has NOT been loaded into the shared planner.

Lifecycle safety remains the mandatory gate before another shared planner reload. CURRENT process tree shows only planner PID `12348` plus its `swe-pro` child and no coder/reviewer OpenCode child, but both `af ps` and direct control-plane read still report `exec_20260902_105806_nz1e2hi6` / `run_20260902_105806_24tfdisq` as `running` since 2026-09-02 with no completion. Treat this as unresolved active-execution visibility, not as proven orphanhood. Do not cancel it or reload the shared planner without separate evidence/authorization.

Nearest compulsory move: keep shared runtime frozen, resolve the stale/active execution visibility gate, then load only the already-tested verifier PATH delta and rerun the same verifier fixture with no other SWE changes. A clean verifier fail-closed on the known-bad semantic case may then advance to the full `implement_issue` repair path and independent oracle.

Historical 2026-09-02 runtime snapshot below is retained as evidence only and is superseded by the 2026-09-07 CURRENT Go-node evidence above; do not use its PIDs, install status, provider state, or blocker wording as current authority:
