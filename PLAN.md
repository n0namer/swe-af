...
6. Start/reload only SWE or the smallest owning target; verify `active/ready` independently of workforce provenance health. **BLOCKED BY CURRENT LIFECYCLE CONTROL**: `agentfield-dev-workforce` exposes `process` but not `reload`; exact `af run swe-planner --port 8800 --detach=true` remains mediation-blocked as opaque. A temporary app-level admission shim `ANTHROPIC_API_KEY=us-e2e-dummy` was set so the legacy bootstrap gate can open while manifest defaults force actual routing to `open_code + openai/DeepSeek-V4`; this does not provide Anthropic routing. Two Coolify restart requests and one stop request were acknowledged but did not materialize (`last_restart_at` stayed null; workforce container ID/uptime unchanged). Hostinger Docker Manager is unsupported on this VPS. Therefore the source is ready for the next actual lifecycle event, but CURRENT loaded process still has not consumed it.
