...
Continue the AgentField takeover gate from the already-tested exact container state; do not add more product behavior. The user has authorized the required canonicalization work. Canonical writeback base is `n0namer/agentfield@4d337c1ae5104418311fcba414a1c2f85c2abb89`; exact tested patch artifact is `/tmp/agentfield-f06-fix/.candidate-writeback.patch`, SHA256 `dad823bc9622e19a4004529d694d892e283d6a240048d1ce758d83638a8066a3`, excluding Python experiment files and `.archsteer/` while including the earlier local `b1602458...` Go SDK layer plus the current proven Go/control-plane/migration delta. **Immediate next move:** when Coding Station health returns, use the existing clean repo session on `4d337c1...` (or recreate an equivalent clean session), apply exactly that artifact, verify staged diff/bytes and rerun focused validators on the canonical session, then publish one candidate branch/PR without redeploy. After exact candidate identity exists, validate frozen SWE via local module replace/full Go suite, rebuild the planner from that exact AgentField commit, and repeat the same F06 SIGTERM/restart fault injection. Resume **FB-0** only when tested AgentField identity = exercised restart identity and the restart oracle remains GREEN.
