...
Nearest compulsory move: **do not reload/reinstall the active Go node, do not canonicalize the current Go runtime tree, and do not enable AUTO/council while exact-tree executable validation is unavailable.** The no-scope-expansion recovery lane is now exhausted for this batch: Coding Station health/readiness actions time out, its underlying containers are Docker-healthy, and direct helper-container diagnostics are operator-mediation `OUT_OF_SCOPE`. Therefore the next executable move requires one explicit scope decision, not another blind retry: either (A) authorize recovery of the existing Coding Station helper runtime/workspace, or (B) authorize a bounded typed Go validation preset on `agentfield-dev-workforce` that can run only the canonical whole-tree gates (`gofmt -l .`, `go build ./...`, `go vet ./...`, `go test -race -count=1 ./...`) against `/src/swe-af/go`. Option B is the smaller blast radius and preferred Pareto route because it validates the exact source without copying 37 dirty files or touching the helper platform. After PASS, rebuild/reinstall the local `swe-planner`, verify installed artifact identity changed, and runtime-accept one `AskUserForm -> HITL_DECISION_EVENT -> unchanged human pause` on the same execution. Telegram project-qualified Mission Control remains active but still lacks a real user-originated `/swe <project> ...` acceptance. Batch 2 adaptive council/deep research and typed Telegram `STEER` remain gated behind these proofs.
