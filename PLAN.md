...
CURRENT BLOCKER:
The active product P0 remains **F16 fail-closed resume on corrupted checkpoint** in exact `/src/swe-af`. The defect is still localized: malformed/unreadable checkpoint is currently conflated with missing checkpoint, allowing `WithResume(true)` to restart from fresh state and potentially repeat a completed non-idempotent effect. The exact SWE/workforce image is still the canonical proven Go validator, but the immediate blocker is now more precise: **`DEV_VALIDATION_CAPABILITY_GAP` in the operator layer**. The existing workforce container is stopped by the normal provenance bootstrap (`Exited 31`), and the typed `debug_clone` path that would safely reuse the same image/toolchain without production reconcile is not enabled for `agentfield-dev-workforce` in the startup-loaded debug-clone registry.

THIS BATCH:
followed BMAD test-design/trace plus systematic-debugging and verification-before-completion. Confirmed `go/Dockerfile` already contains the correct Go/OpenCode toolchain; the `exit 31` guard is external workforce bootstrap, so SWE product code does not need a validation-entrypoint patch. Reuse-first checks found no alternative registered DEV target with Go. The platform-native typed debug-clone mechanism was then selected because it creates a bounded read-only/network-isolated TTL clone of the same workforce image. Both DEV and normal operator planes initially rejected the target with `debug_clone_not_allowed`. Owner readback proved `/var/lib/vps-terminal/targets.runtime.json` had current workforce revision 4 with `canary_file`, `/src`, `/tmp/agentfield-f06-fix`, but no `debug_clone`. A stale-safe runtime-registry update to revision 5 added only `debug_clone`; readback verified it. However clone creation still failed because the running debug-clone manager had loaded its registry at process startup. A normal Coolify restart request did not provide immediate activation proof; later gateway transition showed the manager still did not accept the target. The temporary capability was therefore rolled back stale-safely to revision 6 with all prior fields preserved and `debug_clone` removed; readback verified the rollback. No SWE product source changed.

NORTH-STAR DELTA:
we eliminated the false choice between weakening provenance and inventing a new validator. The correct architecture is now clear: use the existing workforce image as an ephemeral code-only validator through the operator's typed debug-clone path. The remaining gap is activation/registry ownership in `vps-terminal-dev`, not SWE semantics, Go availability or the production provenance guard.

STOP CONDITION:
do not weaken `/opt/reconcile_dev_workspace.sh`, do not publish local SWE code merely to make the workforce start, and do not create an unrelated validator service. First close the operator-layer `DEV_VALIDATION_CAPABILITY_GAP` so `agentfield-dev-workforce` is legally available to the existing debug-clone manager with bounded TTL/cleanup. Then create one ephemeral clone with `/src`, prove exact source + `/usr/local/go/bin/go`, execute the F16 corrupt-checkpoint RED, apply the smallest fail-closed SWE fix only after RED, run targeted DAG/orch tests + full `go test ./... -count=1` + `git diff --check`, perform one adversarial/edge review, and delete the clone with verified absence.
...
The sole next move is to close **`DEV_VALIDATION_CAPABILITY_GAP` in `vps-terminal-dev`**, not to touch SWE semantics yet. The intended mechanism is the existing typed `debug_clone` path against `agentfield-dev-workforce`: it must resolve the same workforce image/toolchain, mount exact `/src` read-only, use bounded TTL/network isolation, and support verified cleanup without invoking the normal provenance bootstrap. Do not rely on a runtime-registry-only capability toggle unless activation/readback proves the debug-clone manager actually consumes it; the last bounded attempt was rolled back cleanly. Once this owner capability is activated through its canonical source/lifecycle and `prepareDebugClone(agentfield-dev-workforce,/src)` succeeds, immediately create one clone, prove `/usr/local/go/bin/go` plus exact `/src/swe-af`, run the F16 corrupt-checkpoint regression to RED, make the smallest fail-closed SWE fix, run targeted DAG/orch + full `go test ./... -count=1` + `git diff --check`, perform one adversarial/edge review, delete the clone with verified absence, and update this PLAN. Do not weaken production provenance, publish local code to satisfy bootstrap, or create an unrelated validator service.
