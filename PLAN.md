...
- Permanent AgentField DEV Coolify application: `edshqtkwskg3lrczekhcmd71`; repo `n0namer/universal-solver`; CURRENT loaded generation is `dr-debug-freeze-20260901` at orchestrator SHA `b7e2f00116358d78d01a73b77aa31d1c2bdfb9d5`.
- CURRENT workforce container is `a8558687cfc2f9c3ffc7b30b57d9c2fbae06cea21fadfd21c3877ef6bd99ba49`, healthy, created 2026-08-31T21:21:27Z. Its startup provenance baseline for SWE remains `58c4e0d19081bc52363c120b7963a34cebb1e894`.
...
- CURRENT exact compose `b78866...` maps Gonka into `OPENAI_API_KEY` + `OPENAI_BASE_URL`, but its SWE start condition still checks only `ANTHROPIC_API_KEY || OPENROUTER_API_KEY`; it does not contain the later `SWE_DEFAULT_RUNTIME=open_code` / Gonka-model overrides seen in other generations. Treat that as `DESIGN_RUNTIME_DRIFT`, not as current proof of the stronger route.
- SWE's own `go/agentfield-package.yaml` still requires one of `ANTHROPIC_API_KEY` or `OPENROUTER_API_KEY`; `OPENAI_API_KEY` is absent from the package preflight contract. This is the smallest proven SWE product/config defect relevant to Gonka enablement.
- SWE Go runtime contains OpenAI-capable paths and `opencode.json` contains explicit OpenAI-compatible provider support. Exact Gonka compatibility and absence of OpenRouter fallback remain unproven until a live `swe-planner` execution succeeds.
- AgentField CURRENT registry reports `swe-planner` / `swe-pro` `inactive/offline` with last heartbeat 2026-08-25, so no current SWE functional claim is allowed.
- Workforce HTTP health is insufficient evidence of SWE readiness: its healthcheck validates provenance, not required SWE node liveness/readiness.
- Operator-plane read/write evidence is currently degraded: the DEV backend reports Runtime File ACI capability, but this chat's callable DEV surface exposes only mediated generic exec; the production typed File ACI registry does not currently expose `agentfield-dev-workforce`. Latest VPS Terminal observation also hit `Portwing transport unavailable`. Do not mutate source blind or work around mediation.
...
5. Apply SWE product code/config changes only in CURRENT `/src/swe-af`; run targeted tests and `cd go && make check` before restart. **LIVE DELTA APPLIED / PARTIAL VERIFY**: shared `/src` was edited through the already-callable `agentfield-dev-runtime-capture` target, which mounts the same `us-af-e2e-runtime-sources` volume as workforce. Exact live delta: (a) `go/agentfield-package.yaml` now admits `OPENAI_API_KEY`; (b) `go/internal/node/node.go` preserves `OPENAI_BASE_URL` into AgentField Go `ai.Config.BaseURL` when `AI_BASE_URL` is unset; (c) `go/internal/node/aiconfig_test.go` adds a regression for that custom base; (d) `opencode.json` adds a generic `openai` provider using `@ai-sdk/openai-compatible`, `OPENAI_API_KEY`, and `OPENAI_BASE_URL`. Workforce readback sees the manifest change. `git diff --check` PASS. Full Go tests are still unexecuted because the callable runtime-capture target has no Go toolchain and DEV workforce process/test exec is mediation-blocked. No GitHub product-code edit or redeploy was used.
6. Start/reload only SWE or the smallest owning target; verify `active/ready` independently of workforce provenance health. **BLOCKED BY PROCESS LANE**: exact `af run swe-planner --port 8800 --detach=true` is still rejected before execution as `REVIEW_REQUIRED: opaque_or_unknown_mutation`, despite server guidance returning `ALLOW` for this scoped start. AgentField registry therefore still reports `swe-planner` offline.
