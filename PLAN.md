...
Nearest compulsory move: do not build a separate middleware service. First close the physical inbound Telegram seam with the live non-mutating `openclaw-routing-canary-20260905`: real operator Reply -> OpenClaw `ReplyToId` -> exact `route` result -> at least two `discuss` turns with no approval callback. Then, once FCM has a healthy planning route, close the stitched SWE seam on a real pending request: AgentField `waiting` -> `governor.pending` -> <=20s Telegram card/message-id mapping -> exact Jarvis route -> explicit final decision -> `/webhooks/approval resolved=true` -> `governor.resolved` -> same execution continues. Only after that call Governor transport L2 PASS.
