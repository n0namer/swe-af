...
CURRENT BLOCKER:
The active AgentField P0 takeover matrix is now **behaviorally GREEN on the exact owner clone**: #1011 notify/admission ordering, #1000 graceful drain/settlement, #1001 async admit-before-persist + pool shutdown terminalization, heartbeat/version compatibility, #1004 persisted generation ownership/deferred instance-scoped reap, and #1046 stale workflow protection. #1046 has deterministic GREEN for both required invariants: recent paired execution activity protects an old workflow, and a heartbeat arriving after candidate selection prevents the final timeout UPDATE. Fresh full `internal/storage -count=1` PASS; fresh `internal/handlers` + `internal/server -count=1` PASS; exact-clone `git diff --check` PASS. Remaining gate is candidate hygiene/exact identity, not a known open P0 behavior defect: experiment-only Python ABA edits still sit dirty without caller-level RED and must be excluded/reverted before the exact local commit; `.archsteer/` is generated output and must not enter the commit. The typed rollback path is temporarily blocked by operator `required_context:context_check_unavailable`. Targeted control-plane `-race` also exceeds the 120s validator ceiling with no verdict; classify as `VALIDATION_BLOCKER`, not race FAIL.
