...
Nearest compulsory move: dedicated bot/account/binding/pairing/outbound are already active; do not revisit BotFather or shared-Jarvis routing. The next acceptance is one **real SWE `waiting` request** delivered as a dedicated `swe` card, followed by at least two non-final discussion turns and then an explicit terminal Reply on that exact card. The terminal Reply must be handled by `swe-governor-telegram` before any model call, resolve `/webhooks/approval` exactly once, move the request to `governor.resolved`, and continue the same `execution_id`. If the SWE/provider path cannot reach HITL, record that external blocker without repairing FCM/provider routing in this workstream; transport remains independently testable and should not regress.
