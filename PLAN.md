...
CURRENT BLOCKER:
The active P0 remains **F16 fail-closed resume on corrupted checkpoint** in SWE exact source. Static owner-source readback proves the defect: `loadCheckpoint()` returns `nil` for both missing/unreadable and malformed JSON; `RunDAG(..., WithResume(true))` treats `nil` as a fresh state and immediately writes a new checkpoint, so a corrupt/partial checkpoint can be misclassified as “no checkpoint” and replay already-completed non-idempotent effects. The immediate blocking gate is now narrowly **`VALIDATION_BLOCKER`: no lawful Go runner is currently available for the exact source snapshot**, so the required executable RED cannot yet be observed.
