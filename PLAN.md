...
THIS BATCH:
followed BMAD test-design/trace plus systematic-debugging and verification-before-completion without touching services or applying a speculative fix. Existing resume tests were checked and cover missing checkpoint plus valid resume, not malformed checkpoint. The discriminating RED is therefore non-duplicate and precise: create malformed `artifacts/execution/checkpoint.json`, call `RunDAG(..., WithResume(true))`, require non-nil error before any `ExecuteFn` invocation, and assert the corrupt checkpoint bytes are unchanged. Reuse-first validator search found no executable Go toolchain on the currently lawful exact-source observation lane: `agentfield-dev-runtime-capture` has no Go at `/usr/local/go/bin/go`, `/usr/bin/go`, `/opt/go/bin/go`, or `/root/go/bin/go`; registered `workbench` also lacks `/usr/local/go/bin/go`; Coding Station remains `Gateway Timeout`; its running coding-runtime has no Go toolchain. A second workbench container was not capability-hopped because authoritative scope binding was absent. No product source was mutated.
