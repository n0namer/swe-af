...
THIS BATCH:
corrected an anti-drift mistake in validator routing. BMAD test-design/trace and verification-before-completion still require an executable RED before the F16 fix, but the project does **not** lack a known validator: the existing SWE/workforce container is the already-proven Go validation environment and previously ran the CURRENT full Go suite successfully. The fresh inventory shows that exact container still exists with the expected `/src`, `/core-src`, `/build`, `/afhome`, and `/workspaces` mounts, but is currently stopped (`Exited 31`). Therefore do not search neighboring workbench/Coding Station containers for Go and do not create a new validation architecture. The validation task is to make the existing proven SWE/workforce test environment available for code-only validation without conflating that with production/service acceptance. No product source was mutated in this correction batch.
