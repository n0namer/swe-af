...
- Runtime cutover is NOT done: live `/src/swe-af` remains old-generation `6f5b4382e6231721f60be7045b9d91fd85e34fb5` plus dirty worktree state. SourceLoop captures emitted there remain old-generation queue items; do not rewrite their base/provenance to `0944bdfb...` until runtime readback proves that generation is actually loaded.
- Old-generation cutoff is frozen: the newest SourceLoop event is `vtchg_6d43abad4cb74bb3951a41d0feccea7f` at `2026-09-15T14:22:52.566Z`; no newer captures were observed. Current SHA-256 of the four parallel-coding files exactly equals their last capture readbacks (`coding.go=3cd4d923...`, `coding_test.go=5248e3f0...`, `coder.go=7ad9de81...`, `sys_coder.txt=0d6464b8...`). The remaining dirty product files are already-classified legacy deltas: `resume_build_id` is replayed on `dev`; planning is preserved as `PARTIALLY_SUPERSEDED / HOLD`; `PLAN.md`, `.source-commit`, and `.agentfield-out-*` are not new product patches.
- Runtime-cutover safety gate is BLOCKED on authoritative idle-state evidence, not source correctness. Workforce process readback shows one live installed `swe-planner` PID and no live OpenCode/git mutator, but AgentField `listActiveExecutions` and recent-execution readbacks both return `Bad Gateway` while gateway health itself is GREEN. Do not replace the live source checkout until authoritative execution state is readable and proves no accepted/running work can be orphaned.
