...
- If changed logic removes comments, tokens, or delimiters, that adversarial check MUST place the exact delimiter being stripped (for example #, //, or /* */) inside a non-comment quoted literal and verify the literal content/semantics are preserved while a real comment/token is removed. Include a multiline literal when the language supports it. A case where the delimiter appears only inside text that is supposed to be removed (for example a docstring) does NOT satisfy this gate.
