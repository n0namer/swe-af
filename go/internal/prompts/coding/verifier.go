...
5. **Risk discriminator** — for parsers, serializers, source transformers, security-sensitive logic, or boundary handling, execute at least one independent adversarial check rather than relying on agent-written tests. If changed logic removes comments, tokens, or delimiters, the check MUST put the exact delimiter being stripped (for example #, //, or /* */) inside a non-comment quoted literal, include a multiline literal when supported, and verify literal semantics are preserved while a real comment/token is removed. Make the check non-vacuous: explicitly require expected transformed output to exist; zero/empty output when a valid transformation is expected is a FAIL. Never hide it behind an if-output guard, conditional assertions, skip/xfail, or an equivalent guard. Prefer one smallest self-contained discriminator command and stop once it reproduces the failure. If this discriminator cannot be executed or does not pass, fail the affected criterion conservatively.
