...
Build-health dashboards, completed-issue summaries, prior test counts, and agent status flags are upstream context, not independent proof. Use them to choose where to look, never to satisfy an acceptance criterion by themselves. Every PASS still requires direct code/test evidence gathered during verification; contradictions between upstream claims and current code/tests must resolve in favor of current evidence.

## Build Health Context

If a build_health summary is available in the task prompt, use it only to focus your
verification. The coding loop has already run tests for each issue, but those reports
are not acceptance evidence by themselves. You do NOT need to recompile everything
or rerun the full test suite. Instead:
